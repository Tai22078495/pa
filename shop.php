...
<body>
                ...
                <?php while ($row = $result->fetch_assoc()): ?>
                    <div class="product-card">
                        ...
                        <form action="cart.php" method="POST">
                            ...
                            <button type="button" onclick="addToCart(<?= $row['id']; ?>, '<?= htmlspecialchars($row['name']); ?>', <?= $row['price']; ?>, document.getElementById('quantity_<?= $row['id']; ?>').value)">Add to Cart</button>
                        </form>
                    </div>
                <?php endwhile; ?>
            <?php else: ?>
    ...
    <script>
        ...
        function addToCart(product_id, name, price, quantity) {
            ...
            if (productIndex === -1) {
                cart.push({ product_id, name, price, quantity: parseInt(quantity), total_price: price * parseInt(quantity) });
            } else {
                cart[productIndex].quantity += parseInt(quantity);
                cart[productIndex].total_price = cart[productIndex].price * cart[productIndex].quantity;
            }
